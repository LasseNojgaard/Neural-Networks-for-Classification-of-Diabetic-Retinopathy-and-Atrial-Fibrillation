...
fileName = 'DENSENET201Oversampled_Histogram_Full'
#fileName = 'VGG16Oversampled_Histogram_Channel'
batch_size = 128
test_batch_size = 128
...
train_datagen = ImageDataGenerator(samplewise_center=True, samplewise_std_normalization=True, histogram_eq_full=True
)

test_datagen = ImageDataGenerator(samplewise_center=True, samplewise_std_normalization=True, histogram_eq_full=True
)

train_generator = train_datagen.flow_from_directory(
        #'/media/lasse/DATA/RetinaBilleder/TrainOversampled',
...
validation_generator = test_datagen.flow_from_directory(
        #'/media/lasse/DATA/RetinaBilleder/Val',
...
test_generator = test_datagen.flow_from_directory(
        #'/media/lasse/DATA/RetinaBilleder/Test',
...
for learn in learningRates:
    file = open(fileName + '_'+format(learn)+'_stats.txt', 'w')
    ...
    history = model.fit_generator(train_generator, steps_per_epoch=train_data_size/batch_size, epochs=epochs, validation_data=validation_generator, validation_steps=val_data_size/batch_size, use_multiprocessing=True, workers=25)
    ...
    plt.savefig(fileName + '_' + format(learn) + '_Accuracy.png', bbox_inches='tight')
    plt.clf()
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'val'], loc='upper left')
    plt.savefig(fileName + '_' + format(learn) + '_Loss.png', bbox_inches='tight')
    plt.clf()
    ...
    plt.savefig(fileName + '_' + format(learn) + '_CM.png', bbox_inches='tight')
    plt.clf()
    ...
    fileReport = open(fileName + '_' + format(learn) + '_Report.txt', 'w')
    fileReport.write("Class" + "  " + "Precision" + "  " + "Recall" + "  " + "F-score" + "  " + "Support" + "  " + "Kappa" + '\n')

    for x in range(0, 6):
        if (x > 4):
            fileReport.write("Aerage" + "  " + format(reportAvarage[0]) + "  " + format(reportAvarage[1]) + "  " + format(
                reportAvarage[2]) + "  " + "  " + format(kappa) + '\n')
        else:
            fileReport.write("Class" + format(x) + "  " + format(report[0][x]) + "  " + format(report[1][x]) + "  " + format(
