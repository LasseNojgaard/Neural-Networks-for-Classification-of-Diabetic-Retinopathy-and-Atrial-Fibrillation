...
train_data_size = 153030
val_data_size = 5184
test_data_size = 5184

#train_data_size = 4064
#val_data_size = 512
#test_data_size = 512
...
num_predictions = 20
sizeOfImage = 224
modelList = ["VGG", "DenseNet"]

for architecture in modelList:
##############################################################################################################
...
##############################################################################################################
    ...
    train_generator = train_datagen.flow_from_directory(
            #'/media/lasse/DATA/RetinaBilleder/TrainUndersampled',
            '/media/lnoejgaard/DataStorage/RetinaBilleder/TrainOversampledResized',
    ...
    validation_generator = test_datagen.flow_from_directory(
            #'/media/lasse/DATA/RetinaBilleder/ValUndersampled',
            '/media/lnoejgaard/DataStorage/RetinaBilleder/ValResized',
            target_size=(sizeOfImage, sizeOfImage),
            batch_size=batch_size
    )#5204
    ...
    opt = keras.optimizers.RMSprop(lr=learn, rho=0.9, epsilon=0.001, decay=decay)
    ...
    history = model.fit_generator(train_generator, steps_per_epoch=train_data_size/batch_size, epochs=epochs, validation_data=validation_generator, validation_steps=val_data_size/batch_size, use_multiprocessing=True, workers=25, callbacks=callbacks_list)
    ...
    file.write('\n')
    file.write("Class" + "  " + "Precision" + "  " + "Recall" + "  " + "F-score" + "  " + "Support" + "  " + "Kappa" + '\n')

    for x in range(0, 6):
        if (x > 4):
            file.write(
                "Aerage" + "  " + format(reportAvarage[0]) + "  " + format(reportAvarage[1]) + "  " + format(
                    reportAvarage[2]) + "  " + "  " + format(kappa) + '\n')
        else:
            file.write(
                "Class" + format(x) + "  " + format(report[0][x]) + "  " + format(report[1][x]) + "  " + format(
                    report[2][x]) + "  " + format(report[3][x]) + '\n')
    ...
    reportAvarage_TwoClass = precision_recall_fscore_support(test_labels_TwoClass, predictions_labels_TwoClass,
                                                             average="macro")
    ...
    plt.savefig(fileName + "_TwoClass" + "_CM" + '.png', bbox_inches='tight')
    plt.clf()
