...
###############################################
matFull = spio.loadmat('/home/lnoejgaard/code/dataFullTrainOver.mat')
matRed = spio.loadmat('/home/lnoejgaard/code/dataRedTrainOver.mat')
mat49 = spio.loadmat('/home/lnoejgaard/code/data49TrainOver.mat')
matLabel = spio.loadmat('/home/lnoejgaard/code/labelTrainOver.mat')

matFullTest = spio.loadmat('/home/lnoejgaard/code/dataFullTest.mat')
matRedTest = spio.loadmat('/home/lnoejgaard/code/dataRedTest.mat')
mat49Test = spio.loadmat('/home/lnoejgaard/code/data49Test.mat')
matLabelTest = spio.loadmat('/home/lnoejgaard/code/labelTest.mat')
...
epochs = 150
files = "AF_ENSEMBLE_DOUBLE"
file = open(files + '.txt', 'w')

archList = ["Fully", "ResNet", "DenseNet"]
#archList = ["ResNet", "DenseNet"]
predictions_labels = [0]*36
predictions_labels = np.array(predictions_labels)

predictions_labelsRes = [0]*36
predictions_labelsRes = np.array(predictions_labelsRes)

predictions_labelsDense = [0]*36
predictions_labelsDense = np.array(predictions_labelsDense)

predictions_labelsFully = [0]*36
predictions_labelsFuly = np.array(predictions_labelsFully)
...
for arch in archList:
    ...
    for train_index, val_index in kf.split(X_train, y_train_temp):
        ...
        if arch == "Fully":
            ...
            #filepath = "weightsFully/weights.{epoch: 02d} - {val_acc: .2f}.hdf5"
        if arch == "DenseNet":
        ...
        if arch == "ResNet":
            ...
            #filepath = "weightsResNet/weights.{epoch: 02d} - {val_acc: .2f}.hdf5"
        ...
        #print("Fit done")
        weightsDenseNet += model.get_weights()

        scores = model.evaluate(X_test, y_test, verbose=0)
        file.write("Model"+ format(numFold) + ":"+"\n")
        file.write('Test loss: ' + format(scores[0]) + '\n')
        file.write('Test accuracy: ' + format(scores[1]) + '\n')
        numFold += 1

        predictionsTemp = model.predict(X_test, batch_size=1)
        
        for ii in range(0, len(predictionsTemp)):
            predictions_labels[ii] += np.argmax(predictionsTemp[ii])
    ...
    if numArch == 1: 
        predictions_labels = np.array(predictions_labels)
        predictions_labels = predictions_labels/5
        predictions_labelsFully = np.round(predictions_labels, 0)
    if numArch == 2: 
        predictions_labels = np.array(predictions_labels)
        predictions_labels = predictions_labels/5
        predictions_labelsRes = np.round(predictions_labels, 0)
    if numArch == 3: 
        predictions_labels = np.array(predictions_labels)
        predictions_labels = predictions_labels/5
        predictions_labelsDense = np.round(predictions_labels, 0)
    numArch += 1


predictions_labels = predictions_labelsDense + predictions_labelsRes + predictions_labelsFully
predictions_labels = np.array(predictions_labels)

predictions_labels = predictions_labels/3
predictions_labels = np.round(predictions_labels, 0)
