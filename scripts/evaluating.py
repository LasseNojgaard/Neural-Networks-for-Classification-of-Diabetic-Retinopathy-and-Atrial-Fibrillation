...
import itertools
from sklearn.metrics import confusion_matrix, precision_recall_fscore_support,cohen_kappa_score,classification_report, accuracy_score
...
##########################################################################################################################################################
#The following code is from the keras file keras/preprocessing/image.py in order to change the image-data-generator so the script works on every computer#
##########################################################################################################################################################
...
class ImageDataGenerator(object):
    ...
    def __init__(self,
                 histogram_eq_channel = False,
                 histogram_eq_full = False,
        ...
        if data_format is None:
            data_format = K.image_data_format()
        self.histogram_eq_channel = histogram_eq_channel
        self.histogram_eq_full = histogram_eq_full
    ...
    def random_transform(self, x, seed=None):
        ...
        if self.histogram_eq_channel:
            for channel in range(x.shape[2]):  # equalizing each channel
                x[:, :, channel] = exposure.equalize_hist(x[:, :, channel])

        if self.histogram_eq_full:
...
##############################################################################################################
#########################################################################################
#The change the parameters in the ImageDataGenerator to change the preprocessing applied#
#########################################################################################
...
test_generator = test_datagen.flow_from_directory(
        '/media/lasse/DATA/RetinaBilleder/TestUndersampled',
        #'/media/lnoejgaard/DataStorage/RetinaBilleder/TestResized',
        target_size=(sizeOfImage, sizeOfImage),
        batch_size=test_batch_size
)#5204

################################################################
#ListOfFiles contains the models that is wished to be evaluated#
################################################################

#ListOfFiles = ["VGG16Oversampled0.0001","VGG16Original0.0001","VGG16Weights0.0001","DENSENET201Oversampled0.0001","DENSENET201Original0.0001","DENSENET201Weights0.0001"]
ListOfFiles = ["TEST0.0001"]

for files in ListOfFiles:
    model = load_model(files+".h5")
    learn = 0.0001
    file = open(files + '_'+format(learn)+'_stats.txt', 'w')
    ...
    test_labels_TwoClass = []
    test_labelsTemp_TwoClass = []
    predictions_labels_TwoClass = []
    ...
    for i in range(0, int(test_data_size / test_batch_size)):
        ...
        #Temp = np.array(test_labelsTemp)

        for ii in range(0, len(test_labelsTemp)):
            test_labels = np.append(test_labels, np.argmax(test_labelsTemp[ii]))
            if np.argmax(test_labelsTemp[ii])==0 or np.argmax(test_labelsTemp[ii])==1:
                test_labels_TwoClass.append(0)
            else:
                test_labels_TwoClass.append(1)
        ...
        for ii in range(0, len(predictionsTemp)):
            ...
            if np.argmax(predictionsTemp[ii])==0 or np.argmax(predictionsTemp[ii])==1:
                predictions_labels_TwoClass.append(0)
            else:
                predictions_labels_TwoClass.append(1)

    file.write("\n" + "Two Class:" + "\n")
    file.write("Acc: ")
    file.write(format(accuracy_score(test_labels_TwoClass, predictions_labels_TwoClass, normalize=True)))
    file.close()

    target_names = ['class 0', 'class 1', 'class 2', 'class 3', 'class 4']
    report = precision_recall_fscore_support(test_labels, predictions_labels)
    reportAvarage = precision_recall_fscore_support(test_labels, predictions_labels, average="macro")
    kappa = cohen_kappa_score(test_labels,predictions_labels, weights='quadratic')
    
    fileReport = open(files + '_' + format(learn) + '_Report.txt', 'w')
    fileReport.write("Class" + "  " + "Precision" + "  " + "Recall" + "  " + "F-score" + "  " + "Support"+ "  " + "Kappa" + '\n')
    for x in range(0,6):
        #file.write("Class" + format(x) + '\n')
        if (x>4):
            fileReport.write("Aerage"  + "  " + format(reportAvarage[0]) + "  " + format(reportAvarage[1]) + "  " + format(reportAvarage[2]) + "  " + "  " + format(kappa)  + '\n')
        else:
            fileReport.write("Class" + format(x) + "  " + format(report[0][x]) + "  " + format(report[1][x]) + "  " + format(report[2][x]) + "  " + format(report[3][x]) + '\n')
    ...
    plt.savefig(files + "_CM" + '.png', bbox_inches='tight')
    plt.clf()

    target_names_TwoClass = ['class a', 'class b']
    report_TwoClass = precision_recall_fscore_support(test_labels_TwoClass, predictions_labels_TwoClass)
    reportAvarage_TwoClass = precision_recall_fscore_support(test_labels_TwoClass, predictions_labels_TwoClass, average="macro")
    kappa_TwoClass = cohen_kappa_score(test_labels_TwoClass, predictions_labels_TwoClass, weights='quadratic')
    ...
    for x in range(0, 3):
        ...
        if (x >= 2):
            fileReport.write(
                "Aerage" + "  " + format(reportAvarage_TwoClass[0]) + "  " + format(reportAvarage_TwoClass[1]) + "  " + format(
                    reportAvarage_TwoClass[2]) + "  " + "  " + format(kappa_TwoClass) + '\n')
        else:
            fileReport.write(
                "Class" + format(x) + "  " + format(report_TwoClass[0][x]) + "  " + format(report_TwoClass[1][x]) + "  " + format(
                    report_TwoClass[2][x]) + "  " + format(report_TwoClass[3][x]) + '\n')
    fileReport.close()
    cm = confusion_matrix(test_labels_TwoClass, predictions_labels_TwoClass)
    plot_confusion_matrix(cm, classes=target_names_TwoClass, title='Confusion matrix, without normalization')
    plt.savefig(files + "_TwoClass" + "_CM" + '.png', bbox_inches='tight')
    plt.clf()
