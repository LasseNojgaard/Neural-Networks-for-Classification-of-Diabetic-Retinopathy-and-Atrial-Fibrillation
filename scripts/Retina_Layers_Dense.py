...
train_generator = train_datagen.flow_from_directory(
        ...
        '/media/lnoejgaard/DataStorage/RetinaBilleder/TrainUndersampledResized',
...
validation_generator = test_datagen.flow_from_directory(
        ...
        '/media/lnoejgaard/DataStorage/RetinaBilleder/ValUndersampledResized',
        target_size=(sizeOfImage, sizeOfImage),
        batch_size=batch_size
)#5204
...
test_generator = test_datagen.flow_from_directory(
        ...
        '/media/lnoejgaard/DataStorage/RetinaBilleder/TestUndersampledResized',
        target_size=(sizeOfImage, sizeOfImage),
...
fileName = 'DenseNet_Layer'
file = open(fileName + '_stats.txt', 'w')

#################################################################################################################
#layerList contains the amount of layers that will be trained, the number of layers are given by 201-LayerList[i]#
#################################################################################################################
layerList = [191, 176, 151, 101, 0]

for layers in layerList:
    file = open(fileName + format(201-layers) + '_stats.txt', 'w')
    learn = 0.00002
    decay = 0
    opti = 'RMS'
    ...
    if opti == 'adam':
        opt = keras.optimizers.Adam(lr=learn, beta_1=0.9, beta_2=0.999, epsilon=decay, decay=0.0, amsgrad=False)
    if opti == 'RMS':
        opt = keras.optimizers.RMSprop(lr=learn, rho=0.9, epsilon=None, decay=decay)
    if opti == 'SGD':
        opt = keras.optimizers.SGD(lr=learn, momentum=0.0, decay=decay, nesterov=True)
    ...
    model.save(fileName + format(201-layers) + '.h5')
    scores = model.evaluate_generator(test_generator, steps=test_data_size / test_batch_size)


    file.write('Number of Layers:' + format(201-layers))
    ...
    plt.savefig(fileName + '_' + format(201-layers) + '_CM.png', bbox_inches='tight')
    plt.clf()
